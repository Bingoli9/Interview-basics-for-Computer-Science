...
tcpdump是linux上的抓包工具，不带任何选项的tcpdump，默认会抓取第一个网络接口，且只有tcpdump进程终止才会停止抓包。
...
3. 监视指定主机的数据包
	...
	```
4. 打印helios与hot或者helios与ace的通信数据包
  ```c++
  tcpdump host helios and \( hot or ace \)
  ```
5. 截获主机hostname发送的（接收的）所有数据包
	```c++
	tcpdump src hostname
	tcpdump dst hostname
	```
6. 监视指定主机和端口的数据包
	```c++
